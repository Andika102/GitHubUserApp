...
class FollowingFragment : Fragment() {
    ...
    private val followingAdapter by lazy { FollowerAdapter() }
    ...
    override fun onCreateView(
        ...
        if (mainViewModel.userFollowersResponse.value?.data == null) {
            ...
            if (favoriteBundle != null) {
                favoriteBundle?.following?.let { setAdapterData(it) }
            } else {
                mainViewModel.userFollowingResponse.observe(viewLifecycleOwner) { response ->
                    if (!response.data.isNullOrEmpty()) {
                        binding.progressBar.visibility = View.GONE
                        followingAdapter.setData(response.data)
                    } else {
    ...
    private fun requestData() {
        if (favoriteBundle != null) {
            ...
            if (favoriteBundle?.following != null) {
    ...
    private fun setupRecyclerView() {
        ...
        binding.followerRecyclerView.adapter = followingAdapter
